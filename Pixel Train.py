...
x_images = np.load('C:/Breast/brain cancer archive/X.npy')

# load labels of shape (5547,1); (0 = no cancer, 1 = cancer)
y_images = np.load('C:/Breast/brain cancer archive/Y.npy')
...
labels_array = ['IDC = 0', 'IDC = 1']
fig, ax = plt.subplots(1,figsize=(5,5))
ax = sns.heatmap(cnf_valid_matrix['neural_network'], ax=ax, cmap=plt.cm.Oranges, annot=True)
ax.set_xticklabels(labels_array)
ax.set_yticklabels(labels_array)
plt.title('Confusion matrix of validation set')
plt.ylabel('True label')
